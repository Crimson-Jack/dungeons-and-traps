<?xml version="1.0" encoding="UTF-8"?>
<tileset version="1.9" tiledversion="1.9.2" name="tilemap_packed" tilewidth="16" tileheight="16" tilecount="168" columns="12">
 <image source="../../img/tilemap_packed.png" width="192" height="224"/>
 <tile id="134">
  <animation>
   <frame tileid="134" duration="100"/>
   <frame tileid="135" duration="100"/>
   <frame tileid="136" duration="100"/>
  </animation>
 </tile>
 <tile id="138">
  <animation>
   <frame tileid="138" duration="100"/>
   <frame tileid="139" duration="100"/>
   <frame tileid="140" duration="100"/>
  </animation>
 </tile>
</tileset>
